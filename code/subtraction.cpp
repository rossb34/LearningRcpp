...
std::vector<double> sub_vec(const std::vector<double>& v1, const std::vector<double>& v2){
    /* subtract two vectors */
    std::vector<double> v(v1.size());
    std::transform(v1.begin(), v1.end(), v2.begin(), v.begin(), std::minus<double>());
    return v;
}

std::vector<double> sub_vec(const std::vector<double>& v1, const double& n){
    /* subtract a vector by a scalar */
    std::vector<double> v(v1.size());
    std::transform(v1.begin(), v1.end(), v.begin(), std::bind2nd(std::minus<double>(), n));
    return v;
}
