...
std::vector<double> add_vec(const std::vector<double>& v1, const std::vector<double>& v2){
	/* add two vectors together */
    std::vector<double> v(v1.size());
    std::transform(v1.begin(), v1.end(), v2.begin(), v.begin(), std::plus<double>());
    return v;
}

std::vector<double> add_vec(const std::vector<double>& v1, const double& n){
	/* add a scalar into a vector */
    std::vector<double> v(v1.size());
    std::transform(v1.begin(), v1.end(), v.begin(), std::bind2nd(std::plus<double>(), n));
    return v;
}
